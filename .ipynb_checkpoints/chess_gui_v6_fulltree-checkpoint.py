...
def layout_and_draw_tree_recursive(surface, node, x, y_center, level, font, current_node):
    """
    Calculates layout, draws piece images instead of circles, updates max_drawn_tree_x and max_drawn_tree_y.
...
def draw_game_tree(surface, root_node, current_node, font):
    ...
    max_drawn_tree_y = 0 # Reset max extent for this draw call
    ...
    required_height = INITIAL_TREE_SURFACE_HEIGHT # Still using fixed size for simplicity
    if tree_render_surface is None or tree_render_surface.get_width() < required_width or tree_render_surface.get_height() < required_height:
         # print(f"Creating/Resizing tree surface to {required_width}x{required_height}")
    ...
    total_tree_height = max_drawn_tree_y + VERT_SPACING # Add padding
    ...
    tree_scroll_y = max(0, min(tree_scroll_y, max_scroll_y)) # Clamp scroll_y

    # --- Autoscroll to the right if needed ---
    if current_node.x + HORIZ_SPACING > tree_scroll_x + tree_panel_rect.width:
        tree_scroll_x = current_node.x + HORIZ_SPACING - tree_panel_rect.width

    # --- Autoscroll to the bottom if needed ---
    if current_node.y + VERT_SPACING > tree_scroll_y + tree_panel_rect.height:
        tree_scroll_y = current_node.y + VERT_SPACING - tree_panel_rect.height
    ...
    if total_tree_height > tree_panel_rect.height:
        ...
        scrollbar_height = tree_panel_rect.height * ratio_visible
        ...
        scrollbar_rect = pygame.Rect(tree_panel_rect.right - 5, scrollbar_y, 5, scrollbar_height)
        pygame.draw.rect(surface, GREY, scrollbar_rect, border_radius=2)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
                ...
                if tree_panel_rect.collidepoint(current_mouse_pos):
                    ...
                    tree_scroll_y += event.y * tree_scroll_speed; tree_scroll_y = max(0, tree_scroll_y); needs_redraw = True
