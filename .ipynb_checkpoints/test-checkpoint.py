...
def draw_coordinates(surface, font):
    ...
    for i in range(8): 
        ...
        text_rect = text_surf.get_rect(center=(COORD_PADDING // 4, BOARD_Y + i * SQ_SIZE + SQ_SIZE // 2))
...
def draw_eval_bar(surface, white_percentage):
    ...
    bar_x = (COORD_PADDING - EVAL_BAR_WIDTH_PX) // 2
    bar_y = BOARD_Y
...
def play_chess960_pygame(start_board_number):
    ...
    displayed_white_percentage = 50.0 # Start at neutral
    ...
    def reset_game(start_pos_num):
        ...
        try:
            ...
            displayed_white_percentage = 50.0
    ...
    while running:
        ...
        # --- Animate Eval Bar ---
        # Determine the target percentage based on the latest available score
        current_score_source = live_raw_score if live_raw_score is not None else (current_node.raw_score if current_node else None)
        if current_score_source is not None:
             target_wp = score_to_white_percentage(current_score_source, EVAL_CLAMP_LIMIT)
             target_wp = 50.0 if target_wp is None else target_wp # Handle None case again just in case
    
        # Interpolate the displayed value towards the target
        smoothing_factor = 0.1 # Adjust this value (0.0 to 1.0) for speed. Smaller is slower/smoother.
        displayed_white_percentage += (target_wp - displayed_white_percentage) * smoothing_factor
    
        # Optional: Snap if very close to prevent tiny oscillations
        # if abs(target_wp - displayed_white_percentage) < 0.1:
        #     displayed_white_percentage = target_wp
        # --- End Animate Eval Bar ---
