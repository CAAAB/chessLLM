...
STOCKFISH_PATH = "/opt/homebrew/bin/stockfish" # Example path
...
def create_eval_plot_surface(eval_history_percent, plot_width_px, plot_height_px):
    ...
    fig.patch.set_facecolor(DARK_GREY) # Use Pygame background color
    ax.set_facecolor(DARK_GREY)

    ax.fill_between(plies, percentages, color='white', alpha=0.9)
    ax.axhline(50, color=ORANGE, linestyle='-', linewidth=1.5)

    ax.set_xlabel("Ply", color='lightgrey')
    ax.set_ylabel("White Win %", color='lightgrey')
    ax.set_title("Win Probability History", color='white', fontsize=10)
    ...
    plt.tight_layout(pad=0.5) # Added tight_layout

    buf = io.BytesIO()
    plt.savefig(buf, format='png', facecolor=fig.get_facecolor(), edgecolor='none')#, bbox_inches='tight') # bbox_inches can sometimes interact badly with tight_layout
    plt.close(fig)
    buf.seek(0)

    try:
        plot_surface = pygame.image.load(buf, 'png').convert() # Load into Pygame surface
    except pygame.error as e:
