...
animating_piece_info = None # Stores {'piece_key', 'img', 'start_pos', 'end_pos', 'start_time'}
...
# --- Sound Loading --- <<< New Function
def load_sounds(path=SOUND_PATH):
    """Loads sound effects from the specified directory."""
    ...
    if not os.path.isdir(path):
        print(f"Error: Sound directory '{path}' not found. Sounds disabled.")
        all_loaded = False
    else:
        for key, filename in sound_files.items():
            ...
            try:
                ...
                print(f"  - FAILED loading {key} ({file_path}): {e}")
                all_loaded = False
    if not all_loaded:
        print("Warning: Some sound effects failed to load.")
    all_sounds_loaded = all_loaded
    return loaded_sounds

# --- Play Sound Helper --- <<< New Function
def play_sound(sound_key):
    """Plays a sound from the loaded dictionary if available."""
...
class GameNode:
    ...
    def get_san(self, board_at_parent):
        ...
        except Exception: return self.move.uci()

    def calculate_and_set_move_quality(self):
        if not self.parent or self.parent.white_percentage is None or self.white_percentage is None: self.move_quality = None; return
        ...
        except ValueError: self.move_quality = None; print(f"Warning: Could not create board from parent FEN: {self.parent.fen}"); return
...
# --- Asynchronous Analysis Manager (Unchanged) ---
class AnalysisManager:
    ...
    def start(self):
        if not self.engine: print("Analysis Manager: No engine provided, cannot start."); return
    ...
    def stop(self):
        ...
        if self._analysis_thread.is_alive(): print("Analysis Manager: Warning - Analysis thread did not stop gracefully.")
    ...
    def _analysis_loop(self):
        ...
        last_request_type = None; last_best_move_params = None
...
def draw_coordinates(surface, font):
    for i in range(8):
        text = chr(ord('a') + i); text_surf = font.render(text, True, COORD_COLOR)
        text_rect = text_surf.get_rect(center=(BOARD_X + i * SQ_SIZE + SQ_SIZE // 2, BOARD_Y + BOARD_SIZE + COORD_PADDING // 2))
        surface.blit(text_surf, text_rect)
    for i in range(8):
        text = str(8 - i); text_surf = font.render(text, True, COORD_COLOR)
        text_rect = text_surf.get_rect(center=(BOARD_X - COORD_PADDING // 2, BOARD_Y + i * SQ_SIZE + SQ_SIZE // 2))
        surface.blit(text_surf, text_rect)

def draw_pieces(surface, board, piece_images, dragging_piece_info, promotion_pending=False, animating_info=None): # <<< Added animating_info
    """Draws the pieces, skipping dragging, promotion pending, and animating pieces."""
    if piece_images is None: return
    animating_from_sq = animating_info['from_sq'] if animating_info else None # <<< Get animating info

    for square in chess.SQUARES:
        # <<< Skip drawing if this square is the *start* square of an animation
        if animating_from_sq is not None and square == animating_from_sq:
            continue
        ...
        if piece:
            ...
            # <<< Skip drawing if this square is the *end* square of an animation (will be drawn by animation code)
            if animating_from_sq is not None and square == animating_info['to_sq']:
                 continue # We don't draw the piece at the destination until animation finishes
...
# <<< New function for animation drawing
def ease_in_out_sine(t):
    """Easing function for smooth animation."""
    return 0.5 * (1 - math.cos(math.pi * t))

def draw_animating_piece(surface, animating_info):
    """Draws the piece being animated via arrow keys."""
    global animating_piece_info # Allow modification to clear it
    if not animating_info: return

    current_time = time.time()
    elapsed_time = current_time - animating_info['start_time']
    progress = min(1.0, elapsed_time / ANIMATION_DURATION)
    eased_progress = ease_in_out_sine(progress) # Apply easing

    start_x, start_y = animating_info['start_pos']
    end_x, end_y = animating_info['end_pos']

    # Interpolate position
    current_x = start_x + (end_x - start_x) * eased_progress
    current_y = start_y + (end_y - start_y) * eased_progress

    # Draw the piece image centered at the interpolated position
    img = animating_info['img']
    img_rect = img.get_rect(center=(int(current_x), int(current_y)))
    surface.blit(img, img_rect.topleft)

    # Check if animation is finished
    if progress >= 1.0:
        animating_piece_info = None # Clear the animation state
        # No need to set needs_redraw here, the main loop redraws anyway after clearing
...
# --- Game Tree Drawing (Stateful, uses global vars) ---
...
def layout_and_draw_tree_recursive(surface, node, x, y_center, level, font, current_node, helpers_visible):
    ...
    if node.children:
        ...
        current_child_y_start = y_center - total_child_height_estimate / 2; next_child_y = current_child_y_start
...
def draw_game_tree(surface, root_node, current_node, font, helpers_visible):
    ...
    if tree_render_surface is None or \
       ...
       tree_render_surface.get_height() < estimated_required_height:
        resize_needed = True; new_width = estimated_required_width; new_height = estimated_required_height
        try: tree_render_surface = pygame.Surface((new_width, new_height))
            ...
            if tree_render_surface: 
                print("Keeping previous tree surface."); resize_needed = False
            else: 
            ...
            error_msg = f"Tree too large ({new_width}x{new_height})! Err: {e}"; error_surf = font.render(error_msg[:80], True, ORANGE)
            surface.blit(error_surf, error_surf.get_rect(center=tree_panel_rect.center)); return
    if tree_render_surface is None:
        pygame.draw.rect(surface, TREE_BG_COLOR, tree_panel_rect); pygame.draw.rect(surface, GREY, tree_panel_rect, 1)
        error_surf = font.render("Tree Surface Error", True, ORANGE); surface.blit(error_surf, error_surf.get_rect(center=tree_panel_rect.center)); return
    tree_render_surface.fill(TREE_BG_COLOR)
    if not root_node: surface.blit(tree_render_surface, tree_panel_rect.topleft, area=pygame.Rect(0,0,tree_panel_rect.width, tree_panel_rect.height)); pygame.draw.rect(surface, GREY, tree_panel_rect, 1); return
    ...
    if current_node and current_node.screen_rect:
        ...
        target_scroll_x = node_rect_on_surface.centerx - tree_panel_rect.width // 2; target_scroll_y = node_rect_on_surface.centery - tree_panel_rect.height // 2
    ...
    if total_tree_width > tree_panel_rect.width:
        ...
        scrollbar_x_ratio = tree_scroll_x / max_scroll_x if max_scroll_x > 0 else 0; scrollbar_x = tree_panel_rect.left + (tree_panel_rect.width - scrollbar_width) * scrollbar_x_ratio
    ...
    if total_tree_height > tree_panel_rect.height:
        ...
        scrollbar_y_ratio = tree_scroll_y / max_scroll_y if max_scroll_y > 0 else 0; scrollbar_y = tree_panel_rect.top + (tree_panel_rect.height - scrollbar_height) * scrollbar_y_ratio
...
def draw_setup_panel(surface, ui_elements, button_font, best_move_button_text, helpers_visible):
    ...
    for rect_key, text, highlighted in buttons:
        button_rect = ui_elements[rect_key]; bg_color = ORANGE if highlighted else BUTTON_COLOR
        pygame.draw.rect(surface, bg_color, button_rect, border_radius=3); pygame.draw.rect(surface, GREY, button_rect, 1, border_radius=3)
...
def save_game_history(current_node, start_time, start_board_number):
    ...
    node_path = get_path_to_node(current_node); moves_san = []; move_num = 1; is_white_move = True
    ...
    try:
        ...
        for i in range(len(node_path) - 1):
            ...
            if move:
                temp_board_for_san.set_fen(parent_node.fen); san = temp_board_for_san.san(move)
                move_prefix = ""
    ...
    except (ValueError, IndexError, AttributeError) as e: print(f"Error generating SAN for PGN: {e}. Saving aborted."); return
    ...
    pgn_moves = ""; ply_count = 0
    temp_board_check_turn = chess.Board(start_fen_for_header, chess960=True); pgn_start_turn_is_white = (temp_board_check_turn.turn == chess.WHITE)
    for san_entry in moves_san:
        pgn_moves += san_entry + " "; ply_count += 1
        if ply_count % 10 == 0 : pgn_moves += "\n"
    try:
        with open(filename, "w") as file: file.write(pgn_header); file.write(pgn_moves.strip()); file.write(" *")
        print(f"Game history saved to {filename}")
    except IOError as e: print(f"Error writing game history to file '{filename}': {e}")
...
def draw_promotion_popup(surface, pos, color_to_promote, mouse_pos):
    ...
    popup_x = pos[0] - PROMOTION_POPUP_WIDTH // 2; popup_y = pos[1] - PROMOTION_POPUP_HEIGHT // 2
    popup_x = max(0, min(popup_x, SCREEN_WIDTH - PROMOTION_POPUP_WIDTH)); popup_y = max(0, min(popup_y, SCREEN_HEIGHT - PROMOTION_POPUP_HEIGHT))
    overall_rect = pygame.Rect(popup_x, popup_y, PROMOTION_POPUP_WIDTH, PROMOTION_POPUP_HEIGHT)
    popup_surf = pygame.Surface(overall_rect.size, pygame.SRCALPHA); popup_surf.fill(PROMOTION_POPUP_BG); surface.blit(popup_surf, overall_rect.topleft)
    ...
    for i, piece_type in enumerate(PROMOTION_PIECES):
        ...
        if img:
            choice_y = overall_rect.top + i * SQ_SIZE; choice_rect = pygame.Rect(overall_rect.left, choice_y, SQ_SIZE, SQ_SIZE)
            choice_rects[piece_type] = choice_rect; img_rect = img.get_rect(center=choice_rect.center)
            if choice_rect.collidepoint(mouse_pos):
                 highlight_surf = pygame.Surface(choice_rect.size, pygame.SRCALPHA); highlight_surf.fill(PROMOTION_HIGHLIGHT_BG); surface.blit(highlight_surf, choice_rect.topleft)
            surface.blit(img, img_rect.topleft)
    promotion_popup_rect_cache = overall_rect; promotion_choice_rects_cache = choice_rects
    return overall_rect, choice_rects

# --- Sound Helper --- <<< New Function
def get_sound_key_for_move(board_before, board_after, move):
    """Determines the appropriate sound key based on the move and board states."""
    if not move: # Should not happen for actual moves, but safety check
        return None

    turn_before_move = board_before.turn
    is_capture_before = board_before.is_capture(move) or board_before.is_en_passant(move) # Include en passant

    # Check results on the board *after* the move
    if board_after.is_checkmate():
        return "checkmate"
    elif board_after.is_stalemate():
        return "stalemate"
    # Check must come *after* mate/stalemate
    elif board_after.is_check():
        return "check"
    # Check captures *before* regular moves
    elif is_capture_before:
        return "white_eat" if turn_before_move == chess.WHITE else "black_eat"
    # Otherwise, it's a regular move
    else:
        return "white_move" if turn_before_move == chess.WHITE else "black_move"
...
def play_chess960_pygame(start_board_number):
    ...
    global promotion_popup_rect_cache, promotion_choice_rects_cache
    global animating_piece_info # <<< Make accessible
    ...
    SOUNDS = load_sounds() # <<< Load Sounds
    ...
    try:
        if STOCKFISH_PATH and os.path.exists(STOCKFISH_PATH):
            ...
            try:
                ...
                message = "Engine error during setup."; engine.quit(); engine = None
        else: message = f"Engine not found at '{STOCKFISH_PATH}'."; print(message)
    ...
    def make_move_and_update(move_to_make):
        ...
        # Store state *before* the move for sound determination
        board_before_move = deepcopy(board) # Keep the deep copy
        ...
        if existing_child:
            ...
            try:
                 board.set_fen(current_node.fen) # Board is now board_after_move
                 live_raw_score = current_node.raw_score
                 temp_san_board.set_fen(parent_node.fen)
                 san = current_node.get_san(temp_san_board)
                 message = f"Played {san} (existing)"
            except ValueError:
                 message = f"Played {move_to_make.uci()} (invalid FEN?)"
                 board = chess.Board(current_node.fen, chess960=True) # Force sync
        else:
            ...
            try:
                temp_san_board.set_fen(parent_fen)
                san = new_node.get_san(temp_san_board)
        ...
        # --- Play Sounds using Helper --- <<< MODIFIED
        # board variable now holds the state *after* the move
    ...
    def reset_game(start_pos_num):
        ...
        nonlocal plot_surface, needs_redraw # <<< Add animation reset
        global tree_scroll_x, tree_scroll_y, promotion_pending

        try:
            board = chess.Board(chess960=True); board.set_chess960_pos(start_pos_num)
            start_fen = board.fen(); print(f"Resetting to Chess960 Position {start_pos_num} (FEN: {start_fen})")
            game_root = None; current_node = None; live_raw_score = None; best_moves_result = None
            highlighted_engine_move = None; analysis_error_message = None; drawn_tree_nodes.clear()
            tree_scroll_x = 0; tree_scroll_y = 0; reset_transient_state(clear_message=False)
            plot_surface = None; promotion_pending = False; animating_piece_info = None # <<< Reset animation

            game_root = GameNode(fen=start_fen, raw_score=None); current_node = game_root
            message = f"Position {start_pos_num} set."
            if analysis_manager: analysis_manager.set_target(start_fen); message += " Analyzing..."

            needs_redraw = True; start_time = datetime.now()
            pygame.display.set_caption(f"Chess960 Analysis - Position {start_board_number}")

        except ValueError as ve:
             print(f"Error setting Chess960 position {start_pos_num}: {ve}"); message = f"Invalid position: {start_pos_num}."
             if game_root is None: board = chess.Board(chess960=True); game_root = GameNode(fen=board.fen()); current_node = game_root
             tree_scroll_x = 0; tree_scroll_y = 0; promotion_pending = False; animating_piece_info = None; needs_redraw = True
        except Exception as e:
            print(f"Error resetting game to position {start_pos_num}: {e}"); message = f"Error setting pos {start_pos_num}."
            if game_root is None: board = chess.Board(chess960=True); game_root = GameNode(fen=board.fen()); current_node = game_root
            tree_scroll_x = 0; tree_scroll_y = 0; promotion_pending = False; animating_piece_info = None; needs_redraw = True

    def reset_transient_state(clear_message=True):
        nonlocal selected_square, dragging_piece_info, legal_moves_for_selected
        nonlocal needs_redraw, message, current_best_move_index
        # Keep animating_piece_info - it's handled separately
    ...
    while running:
        ...
        if analysis_manager:
            ...
            if analysis_result:
                ...
                if error:
                    analysis_error_message = f"Analysis Err: {error}"; print(f"Received analysis error for FEN {result_fen}: {error}")
                    if result_type == 'continuous': live_raw_score = None; live_best_move = None
                    elif result_type == 'best_moves': best_moves_result = analysis_result; highlighted_engine_move = None; current_best_move_index = -1; message = "Best move analysis failed."
                    ...
                    if new_score is not None:
                         ...
                         if current_node.raw_score is None:
                              current_node.raw_score = new_score; current_node.white_percentage = score_to_white_percentage(new_score, EVAL_CLAMP_LIMIT)
                    ...
                    if moves_found:
                        highlighted_engine_move = moves_found[0]['move']; current_best_move_index = 0; num_found = len(moves_found)
                        san = moves_found[0]['san']; score_str = moves_found[0]['score_str']; message = f"Best 1/{num_found}: {san} ({score_str})"
        ...
        for event in pygame.event.get():
            ...
            if promotion_pending: # Handle promotion popup exclusively
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if promotion_popup_rect_cache and promotion_popup_rect_cache.collidepoint(pos):
                        ...
                        if chosen_piece:
                            ...
                            temp_board_promo_check = deepcopy(board) # Use copy to check legality
                            if move in temp_board_promo_check.legal_moves: make_move_and_update(move)
                            else: print(f"Error: Constructed promotion move {move.uci()} is illegal?"); message = "Illegal promotion move?"
                            promotion_pending = False; pending_move = None; promotion_popup_rect_cache = None; promotion_choice_rects_cache = {}; needs_redraw = True
                    else:
                        message = "Promotion cancelled."; promotion_pending = False; pending_move = None; promotion_popup_rect_cache = None; promotion_choice_rects_cache = {}; needs_redraw = True
                elif event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE: message = "Promotion cancelled."; promotion_pending = False; pending_move = None; promotion_popup_rect_cache = None; promotion_choice_rects_cache = {}; needs_redraw = True
            ...
            if event.type == pygame.MOUSEWHEEL:
                ...
                if tree_panel_rect.collidepoint(pos): # Tree Click/Drag
                    if event.button == 1:
                        dragging_tree = True; drag_start_pos = pos; animating_piece_info = None # <<< Cancel animation on click
                        tree_surface_x = pos[0] - tree_panel_rect.left + tree_scroll_x; tree_surface_y = pos[1] - tree_panel_rect.top + tree_scroll_y
                        clicked_node = None
                        ...
                        if clicked_node and clicked_node != current_node:
                             ...
                             try:
                                board.set_fen(current_node.fen); live_raw_score = current_node.raw_score
                                if analysis_manager: analysis_manager.set_target(current_node.fen)
                                reset_transient_state(); message = f"Navigated to ply {current_node.get_ply()}"
                                best_moves_result = None; highlighted_engine_move = None; current_best_move_index = -1; needs_redraw = True
                             except ValueError: message = "Error: Invalid FEN in clicked node."; needs_redraw = True
                elif BOARD_X <= pos[0] < BOARD_X + BOARD_SIZE and BOARD_Y <= pos[1] < BOARD_Y + BOARD_SIZE: # Board Click/Drag
                    if event.button == 1: # Left click
                        reset_transient_state(); animating_piece_info = None # <<< Cancel animation on click
                        sq = screen_to_square(pos)
                        if sq is not None and not board.is_game_over():
                            piece = board.piece_at(sq)
                            if piece and piece.color == board.turn:
                                selected_square = sq; piece_key = ('w' if piece.color == chess.WHITE else 'b') + piece.symbol().upper()
                                img = PIECE_IMAGES.get(piece_key)
                    ...
                    elif event.button == 3: reset_transient_state(); animating_piece_info = None; needs_redraw = True # <<< Cancel animation on click

                # -- Button Clicks -- (No changes needed here for animation/sound)
                elif setup_panel_ui_elements["show_best_move_button_rect"].collidepoint(pos):
                     if not analysis_manager or not current_node or board.is_game_over(): message = "Engine unavailable or game over."; highlighted_engine_move = None; current_best_move_index = -1; best_moves_result = None; needs_redraw = True; continue
                     ...
                     if best_moves_result and best_moves_result.get('fen') == current_fen and best_moves_result.get('moves'):
                         ...
                         else: highlighted_engine_move = None; current_best_move_index = -1; message = "No moves found previously."
                         needs_redraw = True
                     else:
                         message = "Analyzing for best moves..."; highlighted_engine_move = None; current_best_move_index = -1; best_moves_result = None; needs_redraw = True; pygame.display.flip()
                         analysis_manager.request_best_moves(current_fen)
                elif setup_panel_ui_elements["toggle_helpers_button_rect"].collidepoint(pos): helpers_visible = not helpers_visible; message = f"Helpers {'ON' if helpers_visible else 'OFF'}"; needs_redraw = True
                 ...
                 if event.button == 1 and dragging_piece_info: # Finish dragging piece
                    ...
                    from_sq = dragging_piece_info['square']; piece = dragging_piece_info['piece']
                    ...
                    if to_sq is not None and from_sq != to_sq:
                        ...
                        if is_promotion:
                            ...
                            if is_legal_target:
                                promotion_pending = True; pending_move = {'from_sq': from_sq, 'to_sq': to_sq}; promotion_popup_pos = square_to_screen_coords(to_sq)
                                promotion_popup_color = board.turn; promotion_popup_rect_cache = None; promotion_choice_rects_cache = {}; message = "Select promotion piece"; needs_redraw = True; move_made_or_pending = True
                            else: message = "Illegal move"; needs_redraw = True
                        else: # Regular move
                    ...
                    if not move_made_or_pending: selected_square = None; legal_moves_for_selected = []; needs_redraw = True
                 if event.button == 1 and dragging_tree: dragging_tree = False; needs_redraw = True # Finish dragging tree
            ...
            # --- Key Presses --- <<< Modified for Animation
            elif event.type == pygame.KEYDOWN:
                node_changed = False; previous_node = current_node
                move_for_animation = None # <<< Store move causing the change
                is_forward_animation = False # <<< Track direction for piece lookup

                if event.key == pygame.K_LEFT:
                    if current_node and current_node.parent:
                        # Move is the one that led *to* previous_node from current_node (which is now parent)
                        move_for_animation = previous_node.move
                        is_forward_animation = False # Moving backward
                    ...
                    if current_node and current_node.children:
                        # Animate the move from previous_node to the first child
                        move_for_animation = current_node.children[0].move
                        is_forward_animation = True
                    ...
                    if current_node and current_node.parent:
                        parent = current_node.parent; siblings = parent.children
                        try:
                             current_index = siblings.index(current_node)
                             if current_index > 0:
                                 # Animate the move from parent to the new sibling
                                 move_for_animation = siblings[current_index - 1].move
                                 is_forward_animation = True # Treat sibling change like forward
                ...
                # --- Handle Node Change, Trigger Animation, and Play Sound --- <<< MODIFIED Block
                if node_changed:
                    the_move_traversed = move_for_animation # Keep track of the specific move
                    try:
                        # 1. Update board state to the new current node FIRST
                        ...
                        # 2. Determine Board States for Sound (if a move was traversed)
                        if the_move_traversed:
                            board_before_the_move = chess.Board(chess960=True)
                            board_after_the_move = chess.Board(chess960=True)

                            if is_forward_animation:
                                # Moving forward: 'before' is parent (previous), 'after' is child (current)
                                board_before_the_move.set_fen(previous_node.fen)
                                board_after_the_move.set_fen(current_node.fen)
                            else: # Moving backward
                                # 'before' is new parent (current), 'after' is old child (previous)
                                board_before_the_move.set_fen(current_node.fen)
                                board_after_the_move.set_fen(previous_node.fen)

                            # 3. Play Sound based on the traversed move
                            sound_key = get_sound_key_for_move(board_before_the_move, board_after_the_move, the_move_traversed)
                            if sound_key:
                                play_sound(sound_key)

                            # 4. Trigger Animation (using the_move_traversed)
                            animating_piece_info = None # Clear any previous animation
                            from_sq = the_move_traversed.from_square
                            to_sq = the_move_traversed.to_square

                            # Piece lookup uses the board state *before* the move
                            piece = board_before_the_move.piece_at(from_sq)
                            if piece:
                                piece_key = ('w' if piece.color == chess.WHITE else 'b') + piece.symbol().upper()
                                img = PIECE_IMAGES.get(piece_key)
                                if img:
                                    screen_pos_from = square_to_screen_coords(from_sq)
                                    screen_pos_to = square_to_screen_coords(to_sq)
                                    start_pos, end_pos = (screen_pos_from, screen_pos_to) if is_forward_animation else (screen_pos_to, screen_pos_from)
                                    animating_piece_info = {
                                        'piece_key': piece_key, 'img': img,
                                        'from_sq': from_sq, 'to_sq': to_sq,
                                        'start_pos': start_pos, 'end_pos': end_pos,
                                        'start_time': time.time()
                                    }
                                else: print(f"Animation Error: Image not found for {piece_key}")
                            else: print(f"Animation Error: No piece found at {chess.square_name(from_sq)} on board before move")
                        # --- End Sound & Animation Trigger ---

                    except ValueError:
                        message = "Error: Invalid FEN in target node."; print(f"ERROR: Invalid FEN navigation")
                        # Attempt to revert state carefully
                        if previous_node: current_node = previous_node
                        try: board.set_fen(current_node.fen) # Try setting board back
                        except: pass # Ignore if revert also fails
                        animating_piece_info = None # Cancel animation on error
                        needs_redraw = True

                # --- Other Key Actions (no changes needed) ---
                elif event.key == pygame.K_a:
                     if analysis_manager and current_node: message = "Requesting best moves analysis..."; highlighted_engine_move = None; current_best_move_index = -1; best_moves_result = None; needs_redraw = True; pygame.display.flip(); analysis_manager.request_best_moves(current_node.fen)
                ...
                elif event.key == pygame.K_h: helpers_visible = not helpers_visible; message = f"Helpers {'ON' if helpers_visible else 'OFF'}"; needs_redraw = True
        ...
        if not promotion_pending and not animating_piece_info and current_node and board.fen() != current_node.fen:
             try:
                 print(f"Warning: Board desync detected. Forcing board to FEN: {current_node.fen}"); board.set_fen(current_node.fen)
                 live_raw_score = current_node.raw_score; reset_transient_state(clear_message=False); needs_redraw = True
             except ValueError: print(f"CRITICAL ERROR: Invalid FEN in current_node! FEN: {current_node.fen}"); message = "CRITICAL FEN ERROR!"; needs_redraw = True

        # Get last move for highlighting (unchanged)
        last_move_displayed = current_node.move if current_node and current_node.parent else None

        # Update dynamic UI text (unchanged)
        ...
        # --- Redraw Screen ---
        # Force redraw if animation is in progress
        ...
        if needs_redraw:
            ...
            if not promotion_pending:
                # Don't draw last move highlight during animation to avoid clash
                current_last_move_highlight = last_move_displayed if not animating_piece_info else None
                draw_highlights(screen, board, selected_square, legal_moves_for_selected, current_last_move_highlight, highlighted_engine_move)

            # <<< Pass animating_info to draw_pieces
            ...
            # --- Draw Animating Piece (TOP LAYER after board/static pieces) --- <<< New Draw Step
            if animating_piece_info:
                draw_animating_piece(screen, animating_piece_info)

            # --- Draw Promotion Popup (TOP LAYER) ---
    ...
    if engine: 
        try: 
            time.sleep(0.1); engine.quit(); print("Stockfish engine closed.")
