...
def draw_board(surface, board_orientation):
    for r in range(8):
        for c in range(8):
            color = LIGHT_SQ_COLOR if (r + c) % 2 == 0 else DARK_SQ_COLOR
            pygame.draw.rect(surface, color, (BOARD_X + c * SQ_SIZE, BOARD_Y + (7 - r if board_orientation == chess.WHITE else r) * SQ_SIZE, SQ_SIZE, SQ_SIZE))

def draw_coordinates(surface, font, board_orientation):
    for i in range(8):
        text = chr(ord('a') + i)
    ...
    for i in range(8):
        text = str(8 - i if board_orientation == chess.WHITE else i + 1)
        text_surf = font.render(text, True, COORD_COLOR)
        text_rect = text_surf.get_rect(center=(BOARD_X - COORD_PADDING // 2, BOARD_Y + i * SQ_SIZE + SQ_SIZE // 2))
        surface.blit(text_surf, text_rect)

def draw_pieces(surface, board, piece_images, dragging_piece_info, board_orientation):
    ...
    for square in chess.SQUARES:
        ...
        if piece:
            ...
            if img:
                ...
                screen_y = BOARD_Y + (7 - rank if board_orientation == chess.WHITE else rank) * SQ_SIZE
...
def draw_highlights(surface, board, selected_square, legal_moves_for_selected, last_move, engine_move_to_highlight, board_orientation):
    if engine_move_to_highlight:
        s = pygame.Surface((SQ_SIZE, SQ_SIZE), pygame.SRCALPHA); s.fill(ENGINE_MOVE_HIGHLIGHT_COLOR)
        for sq in [engine_move_to_highlight.from_square, engine_move_to_highlight.to_square]:
            ...
            highlight_rect = pygame.Rect(BOARD_X + file * SQ_SIZE, BOARD_Y + (7 - rank if board_orientation == chess.WHITE else rank) * SQ_SIZE, SQ_SIZE, SQ_SIZE)
            surface.blit(s, highlight_rect.topleft)
    if last_move:
        s = pygame.Surface((SQ_SIZE, SQ_SIZE), pygame.SRCALPHA); s.fill(LAST_MOVE_HIGHLIGHT_COLOR)
    ...
    if selected_square is not None:
        ...
        highlight_rect = pygame.Rect(BOARD_X + file * SQ_SIZE, BOARD_Y + (7 - rank if board_orientation == chess.WHITE else rank) * SQ_SIZE, SQ_SIZE, SQ_SIZE)
        s = pygame.Surface((SQ_SIZE, SQ_SIZE), pygame.SRCALPHA); s.fill(HIGHLIGHT_COLOR)
        surface.blit(s, highlight_rect.topleft)
    if legal_moves_for_selected:
        for move in legal_moves_for_selected:
            ...
            radius = SQ_SIZE // 6
            if is_capture: pygame.draw.circle(s, POSSIBLE_MOVE_COLOR, (center_x, center_y), radius + 3, 3)
            else: pygame.draw.circle(s, POSSIBLE_MOVE_COLOR, (center_x, center_y), radius)
            surface.blit(s, (BOARD_X + file * SQ_SIZE, BOARD_Y + (7 - rank if board_orientation == chess.WHITE else rank) * SQ_SIZE))

def draw_board_badge(surface, square, quality, board_orientation):
    ...
    rank = chess.square_rank(square); file = chess.square_file(square)
    square_base_x = BOARD_X + file * SQ_SIZE; square_base_y = BOARD_Y + (7 - rank if board_orientation == chess.WHITE else rank) * SQ_SIZE
...
TEXT_OFFSET_X = 4; INITIAL_TREE_SURFACE_WIDTH = SCREEN_WIDTH * 3
...
def draw_setup_panel(surface, best_move_button_rect, best_move_button_text, toggle_helpers_button_rect, toggle_helpers_button_text, save_game_button_rect, save_game_button_text, toggle_orientation_button_rect, toggle_orientation_button_text, button_font):
    ...
    pygame.draw.rect(surface, BUTTON_COLOR, toggle_orientation_button_rect, border_radius=3)
    pygame.draw.rect(surface, GREY, toggle_orientation_button_rect, 1, border_radius=3)
    toggle_orientation_text_surf = button_font.render(toggle_orientation_button_text, True, BUTTON_TEXT_COLOR)
    toggle_orientation_text_rect = toggle_orientation_text_surf.get_rect(center=toggle_orientation_button_rect.center)
    surface.blit(toggle_orientation_text_surf, toggle_orientation_text_rect)
...
def play_chess960_pygame(start_board_number):
    ...
    setup_panel_ui_elements = {
        ...
        "toggle_orientation_button_rect": pygame.Rect(
            COORD_PADDING + 440, 5, 140, SETUP_PANEL_HEIGHT - 10)
    }
    ...
    pygame.display.set_caption(f"Chess960 Analysis Board - Position {start_board_number}")
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False; break
                ...
                if tree_panel_rect.collidepoint(pos):
                    ...
                    needs_redraw = True
                elif setup_panel_ui_elements["toggle_orientation_button_rect"].collidepoint(pos):
                    board_orientation = chess.BLACK if board_orientation == chess.WHITE else chess.WHITE
                    message = f"Board orientation: {'White down' if board_orientation == chess.WHITE else 'Black down'}"
                    needs_redraw = True
                ...
                if node_changed:
                    try:
                     ...
                     if engine and current_node:
                         ...
                         best_move_san, score_str, raw_score = get_engine_analysis(board, engine, ANALYSIS_TIME_LIMIT * 2.5)
        ...
        if needs_redraw:
            ...
            toggle_orientation_button_text = f"Toggle Orientation ({'White down' if board_orientation == chess.WHITE else 'Black down'})"

            draw_setup_panel(screen, setup_panel_ui_elements["show_best_move_button_rect"], best_move_button_text, setup_panel_ui_elements["toggle_helpers_button_rect"], toggle_helpers_button_text, setup_panel_ui_elements["save_game_button_rect"], save_game_button_text, setup_panel_ui_elements["toggle_orientation_button_rect"], toggle_orientation_button_text, BUTTON_FONT)

            draw_coordinates(screen, COORD_FONT, board_orientation)
            draw_board(screen, board_orientation)
            draw_highlights(screen, board, selected_square, legal_moves_for_selected, last_move_displayed, highlighted_engine_move, board_orientation)
            draw_pieces(screen, board, PIECE_IMAGES, dragging_piece_info, board_orientation)
            if helpers_visible:
                if current_node and current_node.move and current_node.parent and current_node.move_quality:
                    draw_board_badge(screen, current_node.move.to_square, current_node.move_quality, board_orientation)
